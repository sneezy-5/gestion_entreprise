...
class Videoapp():
    def __init__(self):
        ...
        self.receving = StreamingServer(local_ip_address, 9999)
        self.sending = CameraClient( '192.168.1.118', 9999)

        self.t1 = threading.Thread(target=self.receving.start_server)
        self.t1.start()
        time.sleep(2)


        self.t2 = threading.Thread(target=self.sending.start_stream)
        self.t2.start()


        while input("") != "STOP":
            continue
        self.receving.stop_server()
        self.sending.stop_stream()
