...
import {  avanceService } from '@/_services';
...
let goEdit = (id: number)=>{
  router.push({name: 'edit-avance', params:{id:id}})
};
...
const getAll =()=>{
  ...
  avanceService.getAllAvance(filter)
...
const deleteEl = () => {
  avanceService.deleteAvance(ids.value)
      .then((res: { data: { results: any; }; }) => {
        getAll()
    })
    .catch((error: { status: string; }) => {
...
<template>
  ...
  <v-dialog v-model="showDialog" max-width="500px">
    ...
    <div class="flex-end">
      <VBtn to="/create-avance">Ajouter</VBtn>
    </div>
