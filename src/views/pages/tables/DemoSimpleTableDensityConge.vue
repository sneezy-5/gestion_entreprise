...
import {  congeService } from '@/_services';
...
let goEdit = (id: number)=>{
  router.push({name: 'edit-conge', params:{id:id}})
};
...
const getAll =()=>{
  ...
  congeService.getAllConges(filter)
...
const deleteEl = () => {
  congeService.deleteConge(ids.value)
      .then((res: { data: { results: any; }; }) => {
        getAll()
    })
    .catch((error: { status: string; }) => {
...
<template>
  ...
  <v-dialog v-model="showDialog" max-width="500px">
    ...
    <div class="flex-end">
      <VBtn to="/create-conge">Ajouter</VBtn>
    </div>
