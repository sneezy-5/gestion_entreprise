...
let goEdit = (id: number)=>{
  router.push({name: 'edit-fournisseur', params:{id:id}})
};
...
const getAll =()=>{
  ...
  fournisseurService.getAllFournisseurs(filter)
...
const deleteEl = () => {
  fournisseurService.deleteFOurnisseur(ids.value)
      .then((res: { data: { results: any; }; }) => {
        getAll()
    })
    .catch((error: { status: string; }) => {
...
<template>
  ...
  <v-dialog v-model="showDialog" max-width="500px">
    <v-card>
      ...
      <v-card-actions>
        <v-btn  @click="deleteEl" style="color: red;" color="red">Confimer</v-btn>
    ...
    <div class="flex-end">
      <VBtn to="/create-fournisseur">Ajouter</VBtn>
    </div>
