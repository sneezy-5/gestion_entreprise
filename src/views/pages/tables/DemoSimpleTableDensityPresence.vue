...
let goEdit = (id: number)=>{
  router.push({name: 'edit-presence', params:{id:id}})
};
...
const getAll =()=>{
  ...
  presenceService.getAllPresences(filter)
...
const deleteEl = () => {
  presenceService.deletePresence(ids.value)
      .then((res: { data: { results: any; }; }) => {
        getAll()
    })
    .catch((error: { status: string; }) => {
...
<template>
  ...
  <v-dialog v-model="showDialog" max-width="500px">
    ...
    <div class="flex-end">
      <VBtn to="/create-presence">Ajouter</VBtn>
    </div>
