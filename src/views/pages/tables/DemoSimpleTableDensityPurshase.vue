...
import { productService, purshaseService } from '@/_services';
...
let goEdit = (id: number)=>{
  router.push({name: 'edit-purshase', params:{id:id}})
};
...
const deleteEl = () => {
  purshaseService.deletePursharse(ids.value)
      .then((res: { data: { results: any; }; }) => {
        getAll()
    })
    .catch((error: { status: string; }) => {
...
<template>
  ...
  <v-dialog v-model="showDialog" max-width="500px">
    ...
    <div class="flex-end">
      <VBtn to="/create-purshase">Ajouter</VBtn>
    </div>
